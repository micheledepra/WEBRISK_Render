...
## Create territory visual nodes
func create_territory_nodes():
	# For MVP, create simple colored rectangles as territories
	# In a full implementation, these would be proper polygon shapes
	
	var territories_per_row = 7
	var territory_size = Vector2(120, 80)
	var padding = 20
	var start_pos = Vector2(350, 50)
	
	var territory_index = 0
	for territory_id in game_state.territories.keys():
		var row = int(territory_index / territories_per_row)
		var col = territory_index % territories_per_row
		
		var pos = start_pos + Vector2(
			col * (territory_size.x + padding),
			row * (territory_size.y + padding)
		)
		...
		territory_node.position = pos
		territory_node.size = territory_size
	...
	print("Created %d territory nodes" % game_state.territories.size())
