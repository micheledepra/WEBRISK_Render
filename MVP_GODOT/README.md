...
A strategic board game implementation based on the classic Risk board game, ported from a web-based JavaScript version to Godot Engine. Now featuring **SVG-based territory shapes** for accurate geographic representation!

## Overview

This is a single-player implementation of Risk featuring:
- 42 territories across 6 continents with **real SVG shapes**
...
## New: SVG Territory System

Each territory is now rendered using its actual geographic shape from individual SVG files! The map dynamically assembles 42 territory scenes, each with:
- ✅ Unique polygon shape from SVG
- ✅ Dynamic color based on owner
- ✅ Click detection matching exact shape
- ✅ Hover effects and highlighting
- ✅ Positioned at geographic centroid

**[See SVG_TERRITORY_SYSTEM.md for full details →](SVG_TERRITORY_SYSTEM.md)**

## Quick Start

1. Open the project in Godot Engine 4.x
2. Run `Main.tscn` 
3. Territories will automatically load with SVG shapes
4. Play through the 4-phase turn system

To update territory shapes:
```bash
...
│   ├── Territory.tscn         # Territory template (uses Polygon2D)
│   ├── BattleDialog.tscn
│   └── UnitTransferDialog.tscn
├── scripts/         # GDScript game logic
│   ├── Main.gd                # Game initialization, loads SVG data
│   ├── Territory.gd           # Territory behavior with polygon rendering
│   ├── GameState.gd
│   ├── TurnManager.gd
│   ├── PhaseManager.gd
│   ├── CombatSystem.gd
...
│   ├── map_data.json          # Territory connections (updated IDs)
│   └── continents.json        # Continent definitions
├── data/            # Generated data
│   └── territory_polygons.json # SVG polygon data
├── tools/           # Conversion utilities
│   └── svg_to_godot_polygons.py
└── assets/          # Visual assets

territories_svg/     # Individual SVG files (project root)
├── alaska.svg
├── brazil.svg
├── china.svg
└── ... (42 total)
