...
function initializeAttackUI() {
  ...
  window.attackController = {
    ...
    init: function () {
      // Close button
      document
        .getElementById("attack-modal-close")
      ...
      if (this.attackerOptions) {
        const attackerDice =
          this.attackerOptions.querySelectorAll(".dice-option");
      ...
      if (this.defenderOptions) {
        const defenderDice =
          this.defenderOptions.querySelectorAll(".dice-option");
    ...
    openModal: function (attackerId, defenderId) {
      ...
      if (logContainer) {
        logContainer.innerHTML =
          '<div class="battle-log-entry"><div class="battle-log-round">Battle started</div></div>';
      }
    },
    ...
    updateTerritoryInfo: function (attacker, defender) {
      ...
      document.getElementById("attacking-territory-name").textContent =
        this.formatTerritoryName(this.selectedAttacker);
      document.getElementById("attacking-territory-owner").textContent =
        attacker.owner;
      document.getElementById("attacking-territory-armies").textContent =
        attacker.armies;

      // Update defender info
      document.getElementById("defending-territory-name").textContent =
        this.formatTerritoryName(this.selectedDefender);
      document.getElementById("defending-territory-owner").textContent =
        defender.owner;
      document.getElementById("defending-territory-armies").textContent =
        defender.armies;

      // Update army movement elements if they exist
      if (document.getElementById("source-territory-name")) {
        document.getElementById("source-territory-name").textContent =
          this.formatTerritoryName(this.selectedAttacker);
      }

      if (document.getElementById("target-territory-name")) {
        document.getElementById("target-territory-name").textContent =
          this.formatTerritoryName(this.selectedDefender);
      }
    },
    ...
    updateDiceOptions: function (attackerArmies, defenderArmies) {
      // Attacker can use 1 to armies-1 dice
      const maxAttackerDice = Math.max(1, attackerArmies - 1);

      // Enable/disable attacker dice options
      const attackerOptions = document.querySelectorAll(
        "#attacker-dice-options .dice-option"
      );
      ...
      // Defender can use 1 to all their armies
      const maxDefenderDice = Math.max(1, defenderArmies);

      // Enable/disable defender dice options
      const defenderOptions = document.querySelectorAll(
        "#defender-dice-options .dice-option"
      );
    ...
    executeBattle: function () {
      ...
      if (combatSystem) {
        ...
        battleResult = combatSystem.executeBattle(
          this.attackerDice,
          this.defenderDice
        );
    ...
    showBattleResults: function (result) {
        ...
        result.attackerRolls.forEach((roll, index) => {
          ...
          setTimeout(() => {
            ...
            if (
              ...
              result.comparisons[index].attackerWins
            ) {
      ...
      // Update casualties
      document.getElementById("attacker-casualties").textContent =
        result.attackerLosses;
      document.getElementById("defender-casualties").textContent =
    ...
    handleConquest: function () {
      ...
      if (this.armyMovement) {
        ...
        this.armySlider.value = Math.max(1, maxMovable); // Default to max possible

        // Update max armies label
        document.getElementById(
          "max-armies-label"
        ).textContent = `${maxMovable} (max)`;
    ...
    updateArmyMovement: function () {
      ...
      // Update display
      document.getElementById("armies-to-move-display").textContent =
        armiesToMove;
      document.getElementById("source-remaining").textContent =
    ...
    completeConquest: function () {
      ...
      this.showSuccessNotification(
        `You conquered ${this.formatTerritoryName(this.selectedDefender)}!`
      );
    },
    ...
    drawAttackPath: function (attackerId, defenderId) {
      ...
      path.setAttribute(
        "d",
        `M${attackerCenter.x},${attackerCenter.y} L${defenderCenter.x},${defenderCenter.y}`
      );
    ...
    addBattleLog: function (battleResult) {
      ...
      html += `<div class="battle-log-dice">Attacker rolled: ${battleResult.attackerRolls.join(
        ", "
      )}</div>`;
      html += `<div class="battle-log-dice">Defender rolled: ${battleResult.defenderRolls.join(
        ", "
      )}</div>`;
    ...
    completeConquestFallback: function () {
      ...
      console.log(
        `Conquest completed: ${this.selectedAttacker} -> ${this.selectedDefender} with ${this.armiesToMove} armies`
      );
    },
  };
  ...
  Object.entries(gameState.territories).forEach(([territoryId, territory]) => {
    ...
    if (territory.owner === currentPlayer && territory.armies > 1) {
      ...
      element.addEventListener("click", () =>
        handleAttackSourceClick(territoryId)
      );
    }
  });
}
  ...
  if (source.neighbors) {
    source.neighbors.forEach((neighborId) => {
      ...
      if (neighbor && element && neighbor.owner !== currentPlayer) {
        ...
        // Add click event
        element.addEventListener("click", () =>
          handleAttackTargetClick(sourceId, neighborId)
        );
      }
    });
  }
