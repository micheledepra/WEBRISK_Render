class RiskUI {
  ...
  initGame(players, colors = []) {
    ...
    this.phaseSynchronizer.onPhaseChange((data) => {
      console.log(
        `[RiskUI] Phase change: ${data.oldPhase} → ${data.newPhase} (Turn ${data.turn})`
      );
  ...
  initializeVisualsAfterMap() {
    console.log('🎨 Initializing territory visuals after map creation...');
    this.updateAllTerritoryVisuals();
    console.log('✅ Territory visuals initialized successfully');
  }
  ...
  updateTerritoryColor(territoryId) {
    ...
    if (this.colorManager) {
      ...
      console.log(`✅ RiskUI: Updated ${territoryId} using dynamic opacity system`);
      return;
    }
    
    // FALLBACK: Direct DOM manipulation (should rarely be used)
    console.warn('⚠️ RiskUI using fallback - ColorManager not available');
    ...
    if (playerColor) {
      ...
      console.log(`✅ RiskUI fallback: Updated ${territoryId} with ${(opacity * 100).toFixed(0)}% opacity`);
    }
  }
  ...
  setupArmyCountMapIntegration() {
    ...
    console.log('🔗 Army count map integration setup complete');
  }
}
