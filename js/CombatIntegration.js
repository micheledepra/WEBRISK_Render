  ...
  try {
    ...
    if (typeof CombatSystem !== "undefined" && gameState) {
      ...
      if (typeof combatSystem.getPossibleAttackTargets !== "function") {
        console.warn(
          "⚠️ CombatSystem is missing getPossibleAttackTargets method"
        );
      }
    ...
    if (typeof CombatUI !== "undefined" && window.GameUtils) {
      ...
      const initializeCombatUI = () => {
        ...
        const attemptInitialization = (attempt = 0) => {
          try {
            console.log(
              `🔄 Attempting CombatUI initialization (attempt ${attempt + 1})`
            );

            // First, verify DOM structure is ready
            if (!verifyDOMStructure() && attempt < 5) {
              console.warn(
                `⏳ DOM structure not ready yet, will retry in ${
                  (attempt + 1) * 200
                }ms...`
              );
              setTimeout(
            ...
            if (
              ...
              console.log(
                "🔄 Using pre-initialization function from game.html"
              );
              window.ensureAttackModalElements();
            }
            ...
            if (missingElements.length > 0) {
              if (attempt < 5) {
                console.warn(
                  `⚠️ Elements still missing after creation, retrying... Missing: ${missingElements.join(
                    ", "
                  )}`
                );
                setTimeout(
                ...
                console.error(
                  `❌ Failed to create elements after ${attempt + 1} attempts`
                );
              }
            }
    ...
    function ensureCombatUIElements() {
      ...
      if (missing.length > 0) {
        ...
        if (!document.getElementById("attack-modal-attacking-armies")) {
          ...
          attackTerritory.appendChild(el);
          console.log(
            "🔧 Force-created element: attack-modal-attacking-armies"
          );
        }
        ...
        if (!document.getElementById("attack-modal-defending-armies")) {
          ...
          defendTerritory.appendChild(el);
          console.log(
            "🔧 Force-created element: attack-modal-defending-armies"
          );
        }
      }
      ...
      if (missing.length > 0) {
        console.error(
          "❌ Still missing elements after force creation:",
          missing
        );
      } else {
...
function endCurrentCombat() {
  console.log("🛑 Ending current combat session");
  ...
  try {
    ...
    if (window.combatUI && typeof window.combatUI.resetAll === "function") {
      ...
      if (!result) {
        console.warn("⚠️ resetAll returned false");
      }
    }
    // Fallback to reset method
      ...
      if (!result) {
        console.warn("⚠️ reset returned false");
      }
    }
    // Final fallback: manual cleanup
    else {
      console.warn(
        "⚠️ CombatUI reset methods not available, performing manual cleanup"
      );
    ...
    console.log("✅ Combat session ended successfully");
